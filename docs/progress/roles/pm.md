...
- 下一步入口：Owner 已裁决建 `l1-gates`（2026-08-16，采纳 PM 建议，登记异常升级记录第 2 行）→ DevOps 建 CI → Developer 启动实现。
- 收尾状态：未收尾

## 2026-08-15 — 会话摘要（Demo 全代码深读）
- 本次角色：PM（产品经理）
- 动作：产出（Demo 全代码流程深读研究，Owner 指出 PM 未吃透 Demo 属失职后补课）
- 涉及文档：`docs/research/demo-code-deep-dive.md`（新建：快照 3401 行逐段读完 + pip 包 v4.1.4 下载比对）
- 结论：Demo 为五层结构（安装/入口/能力/纪律/SOP）；快照缺 SOP 层（22 个 skills）且缺 forum_functions.py（快照版跑不起来）；MCP 层无批量仿真（批量在 skill 中，含 fingerprint 去重/CSV 断点/并发退避——与 v0.1 验收 #2/#6/#10 机制同构）；本地自相关快算 skill 提供 #6 预判第二路径；get_submission_check 实为相关性+详情拼装（证实 A1 判断）；立项 README 两处结论已修正（见文档 §8）。
- 关键取舍：PRD 已定稿口径与研究结论无冲突，不回阶段；新输入全部列为设计阶段材料。
- 关联迭代：v0.1
- 遗留问题/风险：pip 包活跃演进中（4.1.4 / 2026-08-11），设计阶段应以包版为准而非快照。
- 下一步入口：Owner 确认进设计阶段（上轮遗留）→ Architect 产出设计文档（本研究 §9 为其输入）。
- 收尾状态：未收尾
