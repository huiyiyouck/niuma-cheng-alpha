...
- **待 Owner 裁决（实现阶段开工前）**：是否建最小 `l1-gates`。已登记跨任务待办（P1）。要点——现在是唯一合适的时机窗口：此前无代码无测试、CI 只能空转；此后 N4 要求的 `make test` 已是 CI 要跑的内容，增量成本约十几行 workflow。**维持全程降级模式同样是有效选择，但需显式再裁一次**（DevOps M1 原话：默认漂移成降级而无留痕才是流程缺陷）
...
| P1 | **裁决是否在实现阶段前建最小 `l1-gates`**（只跑单测 + lint，不接触真实平台）。时机窗口 = 设计定稿后、实现阶段开工前——此前项目无代码无测试，CI 只能空转；此后 N4 要求的 `make test` 一键入口即是 CI 要跑的内容，增量成本约十几行 workflow。维持现状（全程降级模式）亦是有效选择，但需**显式再裁一次**，避免默认漂移。若裁定要建，由 DevOps 承接 | Owner（裁决）→ DevOps（若建则执行） | Owner 口述（2026-08-15，Architect 会话提示时机已到）；根源见 DevOps M1 方案 (a) 与 Owner 2026-08-15 的 (b) 裁决（`v0.1.md` 异常升级记录第 1 行） | 未启动 |
